...
function transformToLis(obj) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    liArr.push(`<li>${keys[i]}: ${vals[i]}</li>`);
  }
  return liArr;
}
...
const totalSum = (input) => {
  const flatInput = input.flat();
  const total = flatInput.reduce((acc, val) => {
    acc += val;
    return acc;
  }, 0);
  return total;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i += 1) {
    ...
    input[i].forEach(element => {
      if ((!isNaN(element) && (element % 5 === 0))) {
        subArr.push(Math.pow(2, element));
      }
    });
    newArr.push(subArr);
  }
...
let findShortest = (data) => {
  const htArr = data.sort((charA, charB) => {
    // parseInt
    // charA.height - charB.height
    // add to new array
    // return index 0 of new array - will be shortest char
    parseInt(charA.height) - parseInt(charB.heght);
  });
  console.log(htArr);
  console.log(Object.values(htArr));
  const shortestChar = htArr[0].name;
  return shortestChar;
};
